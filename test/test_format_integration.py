...
# pdf is requested from IGV as svg and converted afterwards, so the batch/snapshot extension is svg
@pytest.mark.parametrize('fmt,expected_ext', [('png', 'png'), ('svg', 'svg'), ('pdf', 'svg')])
def test_cli_format_flag(tmp_path, fmt, expected_ext):
    """`-f FORMAT` reaches the IGV batch script and the requested snapshot paths."""
    captured = {}
    fake = _run_cli(
